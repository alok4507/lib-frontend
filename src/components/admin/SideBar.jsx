      ...
      {SideBarData?.map((data, idx) => {
        return (
          <div key={idx} className="px-4 py-3">
            <p className="text-lg font-bold text-black p-2 border-b-[1px] border-b-slate-300">
              {data?.title}
            </p>
            <div className="mt-3">
              {data?.subCategory?.map((list, idx) => {
                return (
                  <div
                    ...
                    className="flex cursor-pointer p-2 mt-2  items-center justify-start text-black text-lg border-[1px] border-slate-400 rounded-lg"
                  >
                    {list.icon}
